...
private:
 ...
 class PairComp {
   ...
   public:
   bool operator()(Pair_type first_pair, Pair_type second_pair){
     return map_less(first_pair.first, second_pair.first);
   }
 };
...
public:
 ...
 std::pair<Iterator, bool> insert(const Pair_type &val) {
   Iterator it1 = tree.find(val);
   std::pair<Iterator, bool> return_value;

   if (it1 != tree.end()){
    return_value.first = it1;
